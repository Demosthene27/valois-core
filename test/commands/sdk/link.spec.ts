...
describe('sdk:link command', () => {
	...
	describe('sdk:link /path/exists', () => {
		...
		const targetSDKPath = join(__dirname, '../../../', 'node_modules', 'valois-sdk');
	...
	describe('sdk:link ../valois-sdk/sdk', () => {
		const fakeSDKPath = '../valois-sdk/sdk';
		const targetSDKPath = join(__dirname, '../../../', 'node_modules', 'valois-sdk');
		...
		it('should call file system functions with correct parameters', async () => {
			...
			expect(stdout[0]).toContain(`Linked '../valois-sdk/sdk' to '${targetSDKPath}'`);
		});
	});
});
