...
import { ApplicationConfig, HTTPAPIPlugin, MonitorPlugin, utils } from 'valois-sdk';
import {
	getDefaultPath,
	splitPath,
	getFullPath,
...
export default class StartCommand extends Command {
	static description = 'Start Valois Core Node.';
	...
	static flags = {
		...
		config: flagParser.string({
			char: 'c',
			description:
				'File path to a custom config. Environment variable "VALOIS_CONFIG_FILE" can also be used.',
			env: 'VALOIS_CONFIG_FILE',
		}),
		...
		port: flagParser.integer({
			char: 'p',
			description:
				'Open port for the peer to peer incoming connections. Environment variable "VALOIS_PORT" can also be used.',
			env: 'VALOIS_PORT',
		}),
		...
		'api-ws-port': flagParser.integer({
			...
			env: 'VALOIS_API_WS_PORT',
			dependsOn: ['api-ws'],
		}),
		'console-log': flagParser.string({
			description:
				'Console log level. Environment variable "VALOIS_CONSOLE_LOG_LEVEL" can also be used.',
			env: 'VALOIS_CONSOLE_LOG_LEVEL',
			options: LOG_OPTIONS,
		}),
		log: flagParser.string({
			char: 'l',
			description: 'File log level. Environment variable "VALOIS_FILE_LOG_LEVEL" can also be used.',
			env: 'VALOIS_FILE_LOG_LEVEL',
			options: LOG_OPTIONS,
		}),
		'seed-peers': flagParser.string({
			env: 'VALOIS_SEED_PEERS',
			description:
				'Seed peers to initially connect to in format of comma separated "ip:port". IP can be DNS name or IPV4 format. Environment variable "VALOIS_SEED_PEERS" can also be used.',
		}),
		'enable-http-api-plugin': flagParser.boolean({
			description:
				'Enable HTTP API Plugin. Environment variable "VALOIS_ENABLE_HTTP_API_PLUGIN" can also be used.',
			env: 'VALOIS_ENABLE_HTTP_API_PLUGIN',
			default: false,
		}),
		'http-api-plugin-port': flagParser.integer({
			description:
				'Port to be used for HTTP API Plugin. Environment variable "VALOIS_HTTP_API_PLUGIN_PORT" can also be used.',
			env: 'VALOIS_HTTP_API_PLUGIN_PORT',
			dependsOn: ['enable-http-api-plugin'],
		}),
		'http-api-plugin-whitelist': flagParser.string({
			description:
				'List of IPs in comma separated value to allow the connection. Environment variable "VALOIS_HTTP_API_PLUGIN_WHITELIST" can also be used.',
			env: 'VALOIS_HTTP_API_PLUGIN_WHITELIST',
			dependsOn: ['enable-http-api-plugin'],
		}),
		'enable-forger-plugin': flagParser.boolean({
			description:
				'Enable Forger Plugin. Environment variable "VALOIS_ENABLE_FORGER_PLUGIN" can also be used.',
			env: 'VALOIS_ENABLE_FORGER_PLUGIN',
			default: false,
		}),
		'enable-monitor-plugin': flagParser.boolean({
			description:
				'Enable Monitor Plugin. Environment variable "VALOIS_ENABLE_MONITOR_PLUGIN" can also be used.',
			env: 'VALOIS_ENABLE_MONITOR_PLUGIN',
			default: false,
		}),
		'monitor-plugin-port': flagParser.integer({
			description:
				'Port to be used for Monitor Plugin. Environment variable "VALOIS_MONITOR_PLUGIN_PORT" can also be used.',
			env: 'VALOIS_MONITOR_PLUGIN_PORT',
			dependsOn: ['enable-monitor-plugin'],
		}),
		'monitor-plugin-whitelist': flagParser.string({
			description:
				'List of IPs in comma separated value to allow the connection. Environment variable "VALOIS_MONITOR_PLUGIN_WHITELIST" can also be used.',
			env: 'VALOIS_MONITOR_PLUGIN_WHITELIST',
			dependsOn: ['enable-monitor-plugin'],
		}),
		'enable-report-misbehavior-plugin': flagParser.boolean({
			description:
				'Enable ReportMisbehavior Plugin. Environment variable "VALOIS_ENABLE_REPORT_MISBEHAVIOR_PLUGIN" can also be used.',
			env: 'VALOIS_ENABLE_MONITOR_PLUGIN',
			default: false,
		}),
	};
	...
	async run(): Promise<void> {
		...
		this.log(`Starting Valois Core at ${getFullPath(dataPath)}.`);
